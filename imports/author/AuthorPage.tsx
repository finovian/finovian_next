...
const AuthorPage = ({ posts }: FeaturedArticleProps) => {
  return (
    <main className="min-h-screen bg-background px-4 py-12 text-foreground sm:px-8 md:px-20 transition-colors duration-300">
      <section className="mx-auto mb-16 max-w-5xl">
        ...
        <p className="pt-1 pb-3 text-base leading-normal font-normal text-foreground">
        ...
        <h2 className="pt-5 pb-3 text-[22px] leading-tight font-bold tracking-[-0.015em] text-foreground">
          My Philosophy on Investing & Writing
        </h2>
        <p className="pt-1 pb-3 text-base leading-normal font-normal text-foreground">
          Long-term &gt; short-term. Write clearly. Think deeply. Help others.
        </p>
        <p className="pt-1 pb-3 text-sm leading-normal font-normal text-accent underline">
          Want weekly insights? Join the readers list →
        </p>
      </section>

      <section className="mx-auto max-w-5xl">
        <h2 className="mb-6 text-2xl font-semibold text-foreground">Articles by Jay</h2>
        <div className="grid gap-6 md:grid-cols-2">
          {posts.map((i) => (
            ...
            >
              <h3 className="mb-2 text-xl font-semibold text-card-foreground">{i.title}</h3>
              <p className="mb-3 text-sm text-muted-foreground">{i.description}</p>
              <p className="text-sm text-muted-foreground">{new Date(i.publishedAt).toDateString()}</p>
              <a
                href={`${
